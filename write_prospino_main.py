...
try:
    in1 = sys.argv[1]
    in2 = sys.argv[2]
    sqrtS = float(sys.argv[3])
    nlo_string = sys.argv[4]
    error_switch = sys.argv[5]    
except:
    print "input [in1] [in2] [sqrt(S) (TeV)] [LO/NLO] [1/0 (with/without scale variation)]"
    exit()

TeV2GeV = 1000
sqrtS = str(sqrtS * TeV2GeV)
...
if nlo_string == 'LO': 
    ...
    'ERROR: nlo_string has to be LO or NLO'
    print "input [in1] [in2] [sqrt(S) (TeV)] [LO/NLO] [1/0 (with/without scale variation)]"
    exit()
...
!----------------------------------------------------------------------------
  inlo = {inlo}       ! specify LO only[0] or complete NLO (slower)[1]           !
...
!----------------------------------------------------------------------------
  ...
  energy_in = {sqrtS} ! collider energy in GeV                                !
!----------------------------------------------------------------------------

!----------------------------------------------------------------------------
  i_error_in = {error_switch}    ! with central scale [0] or scale variation [1]         !
!----------------------------------------------------------------------------
...
end program main""".format( in1=in1, in2=in2, sqrtS=sqrtS, inlo=inlo, error_switch=error_switch )
